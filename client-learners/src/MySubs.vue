        ...
        <div v-else>
            <md-table>
                <md-table-row>
                    ...
                    <md-table-head>Passed</md-table-head>    
                ...
                <md-table-row v-for="record in records" :key="record.subId">
                    ...
                    <md-table-cell><a>{{ record.unit.split("#")[1] }}</a></md-table-cell>
                    ...
                    <!-- <md-table-cell v-else>
                      <md-button class="md-raised" v-on:click="fireAutoAssess(record.subId)">Auto Assess</md-button>
                    </md-table-cell> -->
                    <md-table-cell>
                      <a v-if="record.hasOwnProperty('result')" :href="'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(record))"
                        target="_blank" :download="record.subId + '.json'"><md-button class="md-raised">View</md-button></a>
                      <b v-else style="text-transform: uppercase; color: grey;">PENDING</b>                      
                    </md-table-cell>             
...
export default {
  ...
  methods: {
    ...
    fireAutoAssess: function(subId) {
      const API = this.$config.API;
      const token = this.$config.Token;
      const uId = this.$config.uId;
      var self = this;
      var aaObject = {
        $class: "org.moocon.core.AutoAssess",
        submission: "resource:org.moocon.core.Submission#" + subId
      };
      this.loading = true;
      //post
      fetch(API + "org.moocon.core.AutoAssess", {
        method: "POST",
        body: JSON.stringify(aaObject),
        headers: new Headers({
